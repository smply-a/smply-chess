...
const PromotionOverlay = ({promotionMoves, orientation, onCancel, onPromote}: Props) => {
    ...
    return (<>
        <div 
            ...
            className="z-10 absolute inset-0 bg-black/30 transition-all duration-200"
            onClick={() => onCancel()}
        />
            
        <div
            className={`z-20 absolute flex ${promRank === Rank.One ? "flex-col" : "flex-col-reverse"}
                bg-white shadow-xl rounded-md overflow-hidden`}
            style={{
                left: `${promFile * 100/8}%`, 
                width: `${100/8}%`,
                height: `${100/2}%`,
        ...
        >
            {promotionMoves.map(move => {
                const newPiece = move.promotion
                if (!newPiece) throw new Error("Expected promotion piece but found none")

                return (
                    <button
                        key={move.to + newPiece}
                        className="w-full h-1/4 flex justify-center items-center text-black hover:bg-gray-200 transition-colors"
